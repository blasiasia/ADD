    ...
    with open(input_metadata_path, 'r') as infile, open(output_metadata_path, 'w') as outfile:
        for line in infile:
            ...
            if len(parts) == 3:
                ...
                model_spoof = parts[2] + parts[3] + parts[4] # 예: - E01 spoof
                ...
                # 수정된 파일명으로 새로운 라인 작성
                new_line = f"{speaker_id} {new_filename} {model_spoof}\n"
                outfile.write(new_line)  # 새로운 metadata 파일에 기록

    print("Metadata 수정 완료.")


input_metadata_path = 'path/to/your/input_metadata.txt'  # 기존 metadata 파일 경로
output_metadata_path = 'path/to/your/output_metadata.txt'  # 수정된 metadata 파일 저장 경로
